...
class ProfileManager {
    ...
    async loadUserProfile() {
        try {
            const response = await AuthHandler.makeAuthenticatedRequest('/Najeekai/api/customer.php', {
                method: 'GET',
                headers: {
    ...
    populateProfileData() {
        ...
        // Update form fields
        const fields = ['firstName', 'lastName', 'middleName', 'username', 'email', 'phoneNumber'];
    ...
    setupEventListeners() {
        ...
        // Handle logout
        const logoutButton = document.querySelector('button i.fa-sign-out-alt').parentElement;
        if (logoutButton) {
    ...
    async handleProfileUpdate(event) {
        ...
        try {
            const response = await AuthHandler.makeAuthenticatedRequest('http://localhost/Najeekai/api/customer.php', {
                method: 'PUT',
    ...
    showAlert(message, type = 'success') {
        this.submitAlert.className = 'rounded-md p-4 mb-4 ' + 
